...
class MatchingRuleUtilSpec extends Specification {

  @Unroll
  def 'matchers lookup returns #matcherClass.simpleName #condition'() {
    expect:
    MatchingRuleUtil.fromMap(map).class == matcherClass

    where:
    map                     | matcherClass      | condition
    [:]                     | EqualsMatcher     | 'if the definition is empty'
    null                    | EqualsMatcher     | 'if the definition is null'
    ...
    [match: 'regex']        | RegexMatcher      | 'if the matcher type is regex'
    ...
    [match: 'include']      | IncludeMatcher    | 'if the matcher type is include'
  }
}
