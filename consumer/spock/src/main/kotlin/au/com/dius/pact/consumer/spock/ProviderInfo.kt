...
data class ProviderInfo @JvmOverloads constructor(
  ...
  private fun httpsKeyStoreProviderConfig(): MockHttpsProviderConfig {
    ...
    return MockHttpsProviderConfig(
  ...
  fun merge(other: ProviderInfo) = copy(
    providerName = providerName.ifEmpty { other.providerName },
  ...
  companion object {
    fun fromAnnotation(annotation: PactTestFor): ProviderInfo {
      ...
      val port = ep.parseExpression(annotation.port, DataType.STRING)?.toString() ?: annotation.port
      return ProviderInfo(
        providerName, annotation.hostInterface, port, pactVersion, providerType,
