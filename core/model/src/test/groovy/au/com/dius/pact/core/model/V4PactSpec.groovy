...
class V4PactSpec extends Specification {
  ...
  def 'test load v4 synchronous messages pact'() {
    given:
    def pactUrl = V4PactSpec.classLoader.getResource('v4-sync-messages-pact.json')
    ...
    pact.interactions.size() == 1
    pact.interactions[0].uniqueKey() == 'm_001'
    pact.interactions[0] instanceof V4Interaction.SynchronousMessages
    pact.interactions[0].description == 'A1'
    pact.metadata['pactSpecification']['version'] == '4.0'
  }
  ...
  def 'test load v4 combined pact'() {
    ...
    pact.interactions[1].contents.matchingRules.toV3Map() == [:]
    pact.interactions[1].contents.generators.toMap(PactSpecVersion.V4) == [content: [a: [type: 'Uuid']]]
    pact.metadata['pactSpecification']['version'] == '4.0'
  }
