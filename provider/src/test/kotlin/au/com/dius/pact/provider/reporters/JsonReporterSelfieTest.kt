...
class JsonReporterSelfieTest {

  private lateinit var reportDir: File

  @BeforeEach
  fun setup() {
    reportDir = createTempDirectory("selfie-json-").toFile()
  }
  ...
  fun `snapshots a successful verification report`() {
    val reporter = JsonReporter("test", reportDir)
    ...
    expectSelfie(normalizeOutput(File(reportDir, "TestProvider.json").readText())).toMatchDisk()
  }
  ...
  private fun normalizeOutput(json: String) = json
    .replace(Regex(""""date"\s*:\s*"[^"]*""""), """"date": "<timestamp>"""")
    .replace(Regex(""""pactJvmVersion"\s*:\s*"[^"]*""""), """"pactJvmVersion": "<version>"""")
}
