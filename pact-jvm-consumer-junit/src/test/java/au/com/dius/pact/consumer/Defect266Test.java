...
public class Defect266Test {
  ...
  public RequestResponsePact getUsersFragment(PactDslWithProvider builder) {
    ...
    assertThat(bodyMatchingRules.keySet(), is(equalTo((Set<String>) Sets.newHashSet("$[0][*].userName", "$[0][*].id", "$[0]",
      "$[0][*].email"))));
    assertThat(bodyMatchingRules.get("$[0][*].userName").getRules().get(0), is(equalTo((MatchingRule)TypeMatcher.INSTANCE)));
    ...
    assertThat(bodyMatchingRules.get("$[0]").getRules().get(0), is(equalTo((MatchingRule)new MaxTypeMatcher(5))));
    assertThat(bodyMatchingRules.get("$[0][*].email").getRules().get(0), is(equalTo((MatchingRule) TypeMatcher.INSTANCE)));
    return pact;
  }
  ...
  public RequestResponsePact getUsersFragment2(PactDslWithProvider builder) {
    ...
    assertThat(bodyMatchingRules.get("$[0][*].id").getRules().get(0),
      is(equalTo((MatchingRule) new RegexMatcher("[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}"))));
    assertThat(bodyMatchingRules.get("$[0]").getRules().get(0), is(equalTo((MatchingRule)new MinTypeMatcher(5))));
    assertThat(bodyMatchingRules.get("$[0][*].email").getRules().get(0), is(equalTo((MatchingRule) TypeMatcher.INSTANCE)));
    return pact;
  }
